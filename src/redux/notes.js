...
export const notesSlice = createSlice({
  ...
  reducers: {
    ...
    setFavoriteNotes(state, action) {
      state.favoriteNotes = state.notes.filter(
    ...
    searchNotes(state, action) {
      state.searchedNotes = state.notes.filter((note) => {
        return note.title.includes(action.payload);
      });
    },
...
export function fetchNotes() {
  return async function fetchNotesThunk(dispatch, getState) {
    ...
    try {
      fetch(
        `https://${process.env.REACT_APP_API_URL}/notes/${
          getState().notes.currentUserId
        }/0`
      )
  ...
  return async function fetchNotesThunk(dispatch, getState) {
    try {
      ...
      let noteDataJSON = {
        ...
        dateModified: (day + " " + month).toString(),
        author: "Hassan",
      };
...
export function editNote(noteId, noteData, action) {
  return async function editNoteThunk(dispatch, getState) {
    ...
    action === "makeFavorite" &&
      dispatch(makeFavorite({ noteId, noteDataJSON }));
    ...
    try {
      ...
      fetch(
        ...
        .then((data) => {
          console.log(data);
          action !== "makeFavorite" &&
            dispatch(EditNoteFromState({ noteId, noteDataJSON }));
        });
    } catch (err) {
      console.log(err);
    }
